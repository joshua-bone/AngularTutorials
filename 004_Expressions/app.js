var app = angular.module('expressions', []);
...
app.controller('myController', function($scope){
  ...
  $scope.add=function(){
    if ($scope.newCapital && $scope.newState){
      $scope.states.push({state: $scope.newState, capital: $scope.newCapital});
      $scope.newCapital = '';
      $scope.newState = '';
    }
  };
  $scope.remove=function(){
    if ($scope.removeString){
      var index = function(){
        for (var i = 0; i < $scope.states.length; i++){
          if ($scope.states[i].state===$scope.removeString || $scope.states[i].capital===$scope.removeString){
            return i;
          }
        }
      }();
